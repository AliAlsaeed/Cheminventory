...
if(isset($_GET['search']) && ($_GET['search'] != '')){
	...
	$s = false;
	$items = $_items->get_co_items($page, $pp);
	$c_items = $_items->count_co_items();
}
?>
<!DOCTYPE HTML>
<html>
...
<body>
	<div id="main-wrapper">
		...
		<div class="wrapper-pad">
			<h2 class="noborder">Check-In Chemical</h2>
			<span class="downtitle">Click a chemical to check-in</span>
			<div id="table-head">
				<form method="post" action="" name="searchf">
                    ...
                    <input style="float: left; width: 150px; font-size:12px; background-color: #65a1b2;
    color: white;
    padding: 5.5px 10px;
    margin: 0px 0;
    border: none;
    border-radius: 4px;
    cursor: pointer;" type="button" value="Reset" onclick="window.location.href='http://www.cheminventory.ca/check-in.php'" />
				</form>
...
?>
					<tr data-id="<?php echo $item->id; ?>">
						<td><?php echo"CHEM-" . $item->id; ?></td>
						<td><?php echo $item->name; ?></td>
						<td><?php echo $_items->get_category_name($item->category); ?></td>
						<td data-tarewight="<?php echo $item->tarewight; ?>" data-chemstate="<?php echo $item->chemicalstate;?>" data-density="<?php echo $item->density;?>"><?php echo $item->qty; ?></td>
                        <td ><?php $cat = $_cats->getusernames($item->owner);
                        echo $cat->name; ?></td>
					</tr>
<?php
						}
