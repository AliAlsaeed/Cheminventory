...
$('document').ready(function() {
	...
	$('table#items-check tbody tr').click(function() {
		...
		if($('tr[data-id='+id+']').next().data('id') != 'y') {
			var cont = $("<tr style=\"display:none\" data-id=\"y\"><td></td><td></td><td></td><td><form method=\"post\" data-saveid=\""+id+"\" name=\""+text+"\"><input type=\"text\" class=\"num\" placeholder=\" New Value \" /></form></td><td><input type=\"submit\" value=\"Save\" class=\"btn blue save "+text+"\" /></td><td></td></tr>");
	...
	function submit_check(from, t) {
		...
		if(from == 'submit') {
            ...
            var tarewight = t.parent().parent().prev().children('td:nth-child(4)').data('tarewight');
            var state = t.parent().parent().prev().children('td:nth-child(4)').data('chemstate');
            var density = t.parent().parent().prev().children('td:nth-child(4)').data('density');
		...
		if(val == '0' || val == '') {
			...
			return false;
		} else if (state != 'gas' && val <= tarewight){
            alert('Please insert a higher number than the Tare Weight '+tarewight);
            return false;
		...
		$.post(phppage, {
			...
			if(data == '2' ) {
				...
				t.parent().parent().prev().children('td:nth-child(4)').fadeOut(200, function() {
					if(actpage == 'in') {
                        if (state == 'solid') {
						  t.parent().parent().prev().children('td:nth-child(4)').html(parseFloat(val) - parseFloat(tarewight)).fadeIn(400);
                        }
                        else if (state == 'liquid') {
                            t.parent().parent().prev().children('td:nth-child(4)').html((parseFloat(val) - parseFloat(tarewight)) / parseFloat(density)).fadeIn(400);
                        }
                        else if (state == 'gas') {
                            t.parent().parent().prev().children('td:nth-child(4)').html(parseFloat(prev_value) - parseFloat(val)).fadeIn(400);
                        }
                    }
					else
						t.parent().parent().prev().children('td:nth-child(4)').html(val).fadeIn(400);
				});
			}else{
